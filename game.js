...
function completeChapter2() {
    ...
    const avgRate = (totalATP / 30).toFixed(2);

    // Context: Healthy mitochondria produce 900-1500 ATP in 30 seconds (30-50 per second)
    // Maya's impaired mitochondria produce 12-30 ATP in 30 seconds (0.4-1.0 per second)
    const message = totalATP < 20
        ? `You produced only ${totalATP} ATP in 30 seconds (${avgRate} ATP/sec average).\n\nHealthy mitochondria produce 30-50 ATP per SECOND - that's 900-1,500 ATP in 30 seconds!\n\nMaya's cells produce only ${((totalATP/900)*100).toFixed(1)}% of normal. This is why she's always exhausted - her cells are starving for energy.`
        : `You produced ${totalATP} ATP in 30 seconds (${avgRate} ATP/sec average)!\n\nThat's better than Maya's cells usually manage, but still only ${((totalATP/900)*100).toFixed(1)}% of what healthy mitochondria produce (900-1,500 ATP in 30 seconds).\n\nLet's see what the doctor says about this...`;
